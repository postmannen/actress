...
type Process struct {
	...
	// Ensures readyCh is closed at most once to avoid races/double-close
	readyOnce sync.Once
...
func NewRootProcess(ctx context.Context, fn ETFunc, conf *Config) *Process {
	...
	p := Process{
		...
		StaticEventCh:       make(chan Event, 10),
		ErrorEventCh:        make(chan Event, 10),
		TestCh:              make(chan Event, 10),
		DynamicEventCh:      make(chan Event, 10),
...
func RegisterProcessesInESProcesses(p *Process, pi *registerProcessInfo) {
	...
	// 		syncP := NewProcess(p.Ctx, p, EDSync,  EDSyncFn(syncCh))
	// 		syncP.Act()
	// 		defer syncP.Stop()
	//
...
func (p *Process) AddEvent(event Event) {
	...
	if event.DstNode != p.Config.NodeName && event.DstNode != "" {
		...
		//	slog.Debug("AddEvent", "on", p.Config.NodeName, "wrapping event nr", event.Nr, "in ETRemote, event.DstNode", event.DstNode)
		//}
...
func (p *Process) addEventStatic(event Event) {
	//if slog.Default().Enabled(context.TODO(), slog.LevelDebug) {
	//	slog.Debug("addEventStatic", "[1 of 2] before adding event to StaticEventCh on", p.Config.NodeName, "event", CopyEventFields(&event))
	//}
...
func (p *Process) IncrementEventNr() int {
	if p.EventNr == nil {
		p.EventNr = newEventNr(0)
	}
	p.EventNr.mu.Lock()
	defer p.EventNr.mu.Unlock()
	p.EventNr.nr++
	return p.EventNr.nr
}
