		...
		fn := func() {
			for {
				...
				case evGetInit := <-p.InCh:
					// The initial httpGet event should contain:
					// - r.Host to connect to
					// - uuid of the listening process to send data back to.
					// - uuid of the dynamic process who owns the destination connection
					//   so we have somewhere to send an event to terminate it.
