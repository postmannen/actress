...
impl Node {
    // Explanation of the argument syntax:
    // - &str is a string slice — a reference to UTF-8 text somewhere in memory (not owned)
    // - 'static is a lifetime — it tells the compiler this reference is valid for the entire program duration
    pub const fn new(s: &'static str) -> Node {
        // Cow<'_, str> = "Clone on Write" smart pointer.
        // Wraps borrowed data (zero-cost) but can lazily clone into owned heap data
        // when mutation is needed. This lets callers pass either &'static str or owned
        // strings transparently — no allocation for immutable reads, clone only on write.
        Node(Cow::Borrowed(s))
    }
...
pub fn NewEvent(et: EventName, opts: Vec<EventOpt>) -> Event {
    let mut ev = Event { Name: et, ..Default::default() };
    for opt in opts {
        opt(&mut ev);
    }
    ev
