						...
						func(ctx context.Context, p *Process) func() {
							return func() {
								...
								case <-p.InCh:
									...
									t.Logf("successfully deleted process: %v\n", dyn2EVType)
...
func TestDynamicProcessReaderWriter(t *testing.T) {
	...
	NewProcess(ctx, rootp, ETTest, edTestFn).Act()
...
func TestEDSync(t *testing.T) {
	...
	NewProcess(ctx, rootp, EDSync, EDSyncFn(syncCh)).Act()

	rootp.AddEvent(Event{Name: EDSync,
		Data: []byte("test")})
